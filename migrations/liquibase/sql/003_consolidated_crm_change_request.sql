...
--
-- BUG FIXES INCLUDED
--   - Ensure referenced tables have UNIQUE (tenant_id, id) when using composite FKs
--     of the form (tenant_id, <id>) -> (<tenant_id>, <id>).
--   - Ensure pipeline_stage has UNIQUE (tenant_id, pipeline_id, id) for composite FK
--     enforcement of "stage belongs to pipeline".
-- ======================================================================
...
-- ----------------------------------------------------------------------
-- Many of the foreign keys below reference (tenant_id, id). Postgres requires that
-- the referenced table have a PRIMARY KEY or UNIQUE constraint on the exact same
-- column list and order. These UNIQUE constraints are structural integrity helpers
-- and do not change the logical meaning of the tables.
ALTER TABLE dyno_crm.pipeline
...
-- ----------------------------------------------------------------------
-- Ensures a stage can be referenced by (tenant_id, pipeline_id, stage_id) as a unique key.
-- This enables composite FKs that prove the stage belongs to the pipeline.
ALTER TABLE dyno_crm.pipeline_stage
...
-- Fast lookup by company for deal lists / account views
...
--   contact_role provides a lightweight role label per association.
CREATE TABLE IF NOT EXISTS dyno_crm.deal_contact (
    ...
    -- Optional role label (e.g., "DECISION_MAKER", "CHAMPION", "PROCUREMENT")
    ...
    -- Prevent duplicate link rows for the same deal-contact pair
    CONSTRAINT ux_deal_contact UNIQUE (tenant_id, deal_id, contact_id)
);

-- Fast lookups by contact (reverse navigation: "show me deals this contact is on")
CREATE INDEX IF NOT EXISTS ix_deal_contact_contact
    ON dyno_crm.deal_contact (tenant_id, contact_id);

-- Fast lookups by deal (load participants for a deal)
