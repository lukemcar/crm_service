...
def auth_jwt(
    ...
    async def _auth_jwt(
        ...
        try:
            logger .info("Received JWT token for validation")
            ...
            # dubug info: log all claims
            if logger .isEnabledFor(logging.DEBUG):
                for claim, value in payload.items():
                    logger .debug("JWT claim: %s = %s", claim, value)
            ...
            logger .error("JWT validation failed: Token has expired - %s", e)
            raise HTTPException(status_code=401, detail="Token has expired")
        except jwt.JWTError as e:
            logger .error("JWT validation failed: Invalid token - %s", e)
            raise HTTPException(status_code=401, detail="Invalid token")
        except HTTPException:
            raise
        except Exception as e:
            logger .error("JWT validation failed: Unexpected error - %s", e)
            raise HTTPException(
                status_code=500,
